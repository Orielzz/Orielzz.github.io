...
const preencheFicha = filme => {
    document.querySelector(".catalogo").innerHTML += `
      ...
      <div class ="box1">
        ...
        <p>Títulos semelhantes: ${filme.titulosSemelhantes}</p>
        <p>Classificação: <span id="etaria">${filme.classificacao} </span></p>
       </div>

       <div class = "box2">
        <img src="${filme.figura}" alt="" srcset="">
        <p>Elenco: ${filme.elenco}</p>
        </div>
        <p id = "opinion">Opiniões: </p>
  ...
  const atribuiClasseFaixaEtaria = () => {
    ...
    for (let i = 0; i < elementosFaixaEtaria.length; i++) {
      const elementoFaixaEtaria = elementosFaixaEtaria[i];
      
      if (elementoFaixaEtaria.textContent <= 14) {
        elementoFaixaEtaria.classList.add("verde");
      } else if (elementoFaixaEtaria.textContent < 18) {
        elementoFaixaEtaria.classList.add("amarelo");
      } else {
        elementoFaixaEtaria.classList.add("vermelho");
      }
    }
  }
...
const escrevefilmes = acervo =>{
    
    for(let i = 0 ; i<acervo.length;i++){
        preencheFicha(acervo[i])
        escreveOpinioes(acervo[i].opinioes)
        
        
    }
    
}
const descreveOpinioes = objeto =>{
    console.log(objeto.rating)
    document.querySelector(".catalogo").innerHTML += `<div id="descricao">
    &emsp;         Rating: ${classificacaoComEstrelas(objeto.rating)}<br>
    &emsp;            Descrição: ${objeto.descricao}<br><br>
    </div>
    `
}

const escreveOpinioes = array =>{
    for (let i = 0; i < array.length; i++) {
       descreveOpinioes(array[i]);
    }
}
